...
Output: 2
Tags
searchingmatrixhash tableGooglecache
"""
strArr= ["01111", "01101", "00011", "11110"] # Output: 3
# strArr= 
# ["01111", 
#  "01101", 
#  "00011", 
...
def BitmapHoles(strArr):
    ...
    def dfs(a,b):
        deltaC = [0, 1, 0, -1]
        deltaR = [-1, 0, 1, 0]
        if 0<=a< len(strArr) and 0<=b< len(strArr[0]) and strArr[a][b] =='0':
            strArr[a] = strArr[a][:b]+'x'+strArr[a][b+1:]
            for i in range(4):
                na,nb = a+deltaR[i], b+deltaC[i]
                dfs(na,nb)

    for a in range(len(strArr)):
        for b in range(len(strArr[0])):
            if strArr[a][b] == '0':
                cnt +=1
                print(strArr)
                dfs(a,b)
